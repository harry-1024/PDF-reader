...
def load_existing_matrix_data(existing_file):
    """
    Reads the already formatted matrix file and reconstructs row data + product metadata.
    """
...
def write_formatted_matrix(all_rows, product_meta_map, output_name, backup_name):
    ...
    matrix = matrix[["Contaminant", "CAS #", "Solid [1]"] + existing_products]
    ...
    ws.cell(row=row_product_name, column=4, value="Product Name")
    ws.cell(row=row_product_id, column=4, value="Product Identification Number")
    ws.cell(row=row_spec_gravity, column=4, value="specific gravity")

    ws.cell(row=row_header, column=1, value="Contaminant")
    ws.cell(row=row_header, column=2, value="CAS #")
    ws.cell(row=row_header, column=3, value="Solid [1]")
    ...
    for row_idx, (_, row) in enumerate(matrix.iterrows(), start=data_start_row):
        ...
        ws.cell(row=row_idx, column=3, value=row["Solid [1]"])
...
# Main
# ----------------------------
def process_all_sds_incremental(folder_path, output_name="Master_SDS_Matrix_Formatted.xlsx"):
    ...
    for filename in pdf_files:
        ...
        try:
            ...
            rows, meta = parse_sds_file(pdf_path)
...
process_all_sds_incremental(sds_folder)
